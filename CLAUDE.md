...
ユーザーからの依頼に基づいてHTMLベースのデザインモックを生成する際の手順：

```
依頼例: "docs/design/screens/index.md を読み込んで、
...
3.  **実装を意識した構成**
    -   このHTMLモックは**デザイン確認用**であり、実際は `tech-stack.md` の技術で実装する
    -   対象技術で実装しやすい構成を心がける
    -   コードにはヒントとなるコメントを入れる

4.  **出力先**: `docs/design/screens/html/`
...
7.  **デザイン**
    -   index.mdのデザイン方針（トーン、カラーパレットの方向性等）に従う
...
10. **アセットについて**
    -   画像などのアセットは後からユーザーが追加する
    -   今はアセットが無くても404エラーを気にせず、プレースホルダーを配置してコーディング

#### 出力物

1. **HTMLプロジェクト**: `docs/design/screens/html/` 配下
2. **README.md**: HTMLプロジェクトのルートに作成
...
画像等のアセットが必要な場合は、以下のフォーマットで**必要なアセットをすべてリストアップ**する：
...
- サイズ: 1920x1080px
- 形式: JPG
- 説明: トップページのヒーローセクション背景。抽象的なグラデーション。
```
...
### 生成ファイル
- docs/design/screens/html/ 配下に X 画面分のHTMLを生成
- docs/design/screens/html/README.md にデザイン設計を記載

### 対応状況
- レスポンシブ対応: PC / スマホ
- デザイン: index.mdの方針に従って設計
- 導線: 画面間リンク設定済み
- 実装ヒント: コメントで記載
